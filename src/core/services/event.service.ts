...
class EventsService {
  ...
  async getFilteredEvents(data: IEventFilter) {
    const response = await this.axios.get(
      `${import.meta.env.VITE_API_DEV_URL}/events/filtered?city=${
        data.cities
      }&category=${data.categoryId}&dateTo=${data.dateRange?.to}&dateFrom=${
        data.dateRange?.from
      }`
    );
    return response.data;
